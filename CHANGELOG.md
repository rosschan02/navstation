...
- 将「行为趋势 / 24 小时分布 / 来源页面 / 热门分类」整体下沉到页面底部，优先展示核心统计与排行
...
#### 行政区域查询按钮合并
- 合并「行政区域速查」与「本地行政区查询」两个按钮为统一的「行政区域查询」
- 弹窗内通过"在线查询 / 本地查询"切换两种搜索模式
- 在线查询：按省份 + 关键词查百度 Place API v3，展示四级区划代码
...
#### 行政区域速查升级（百度 Place API v2 → v3）
- API 端点升级至 `https://api.map.baidu.com/place/v3/region`
- 新增 `town`（街道/乡镇名称）和 `town_code`（9 位行政区划代码）字段
- 搜索结果列表新增街道层级显示
- 详情页新增「行政区划代码」区块：由 `town_code` 直接推算省/市/区/街道四级代码，无需本地数据库
- 每级代码可点击一键复制

### 修改文件

```
src/app/api/regions/search/route.ts   — 升级 v3，新增 town/town_code
src/components/AdministrativeRegionQuickSearchModal.tsx — 详情页展示区划代码链
README.md
```
