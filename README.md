...
- 首页搜索区新增固定 `英德市` 天气摘要卡，点击卡片可直接打开并加载英德市详细天气
- 首页天气摘要请求默认不写入天气查询统计，避免将被动展示计入手动查询数据
- 后台站点设置新增“站点图标”上传项，支持自定义浏览器 favicon（含 `.ico`）

## 最近更新（2026-03-06）

- 首页导航站点卡片改为宽屏自适应网格，2K/4K 分辨率下会基于卡片最小宽度自动展示更多列
- 软件下载页同步改为宽屏自适应网格，高分辨率下不再固定最多 3 列
- 首页与软件下载页内容区最大宽度放宽，并保持左对齐，解决侧边栏与正文之间大面积留白
- Next.js 16 请求拦截文件由 `src/middleware.ts` 迁移到 `src/proxy.ts`，消除弃用警告
- `package.json` 显式声明 `npm` 为包管理器，修复 Docker / 本地安装状态漂移导致的 `qrcode` 构建失败

## 最近更新（2026-03-03）

- 合并「行政区域速查」与「本地行政区查询」为统一的「行政区域查询」按钮，弹窗内通过"在线查询/本地查询"切换两种模式
- 本地行政区 API：`GET /api/admin-divisions`，支持关键词搜索、详情上级链路、下级区域钻取
- 四级行政区数据库迁移与导入脚本：`011_add_admin_divisions.sql` + `scripts/import-admin-divisions.sql`
- 在线查询模式使用百度 Place API v3，新增街道（乡镇）层级及 `town_code`（9位行政区划代码）
- 详情页直接由 `town_code` 推算省/市/区/街道四级区划代码，无需本地数据库，每级代码可一键复制
- 新增「天气速查」弹窗：支持中文地区名或 `district_id` 查询，默认英德市（`441881`）
- 新增天气数据缓存与行政区划映射：默认查询走服务端缓存，支持导入 `weather_district_id.csv` 映射中文地名到 `district_id`
- 天气弹窗完成视觉重构（渐变 Hero + 指标卡片 + 分类图标配色），并优化 7 日预报不换行显示
- 天气弹窗新增横向 24 小时预报与空气指数模块（AQI 等级 + PM2.5/PM10/NO2/SO2/O3/CO 指标）
- 软件下载页面每个软件卡片新增“更新时间”显示（使用上传时间 `created_at`）
- 优化长者友好可读性：行政区域速查详情页与电话本速查列表字体整体增大，短码加大显示
...
- **首页导航**: 所有站点按分类分组展示，左侧边栏分类筛选，支持全文搜索，显示服务器局域网 IP 地址；搜索区右侧常驻展示英德市当天天气摘要，点击可直接进入并加载英德市详细天气
